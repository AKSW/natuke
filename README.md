...
6.1. ```knn_dynamic_benchmark.py```: runs the experiments using the generated knowledge graph, considering the parametrers set on the main portion of the code;
6.2. ```knn_dynamic_benchmark_splits.py```: runs the experiments using the splits tailored for the BiKE (First International Biochemical Knowledge Extraction Challenge: http://aksw.org/bike/) challenge;
